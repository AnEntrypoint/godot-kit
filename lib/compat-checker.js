...
function checkFile(filePath) {
  const warnings = [];
  let src;
  try { src = fs.readFileSync(filePath, 'utf8'); } catch (e) { return warnings; }
  const lines = src.split('\n');
  for (let i = 0; i < lines.length; i++) {
    const line = lines[i];
    for (const { pattern, message } of DEPRECATED_PATTERNS) {
      if (pattern.test(line)) {
        warnings.push({ file: filePath, line: i + 1, message, text: line.trim() });
      }
    }
  }
  return warnings;
...
module.exports = { checkFile, checkDir, formatWarnings, DEPRECATED_PATTERNS };

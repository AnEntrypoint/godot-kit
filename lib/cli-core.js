...
function registerCoreCommands(program) {
  ...
  program.command('validate').description('Lint all .gd files with gdlint (Godot 4.x) and check for Godot 3→4 migration issues')
    .option('--dir <path>', 'Project directory to scan', '.')
    .option('--strict', 'Exit 1 on migration warnings (default: warnings print but exit 0)')
    .action((opts) => {
      ...
      console.log('\nChecking for Godot 3→4 migration issues...');
      ...
      else { console.log('\x1b[32mNo migration issues found.\x1b[0m'); }
      if (lintFailed || (opts.strict && warnings.length)) process.exit(1);
    });
  ...
  program.command('watch').description('Watch .gd files and hot-reload running game on change')
    ...
    .action(async (opts) => {
      ...
      const watcher = chokidar.watch(['**/*.gd', '**/*.tscn', '**/*.tres'], { ignored: /node_modules/, ignoreInitial: true });
  ...
  inputMapCmd.command('list').description('List all input actions from project.godot')
    .action(() => {
      const fs = require('fs');
      const projPath = path.join(process.cwd(), 'project.godot');
      let raw;
      try { raw = fs.readFileSync(projPath, 'utf8'); } catch { console.error('project.godot not found. Run from a Godot project directory.'); process.exit(1); }
      let inInput = false; let found = false;
      raw.split('\n').forEach(l => {
        if (l.trim() === '[input]') { inInput = true; return; }
        if (inInput && l.startsWith('[')) { inInput = false; return; }
        if (inInput && l.includes('=')) { found = true; console.log(l.trim()); }
      });
      if (!found) console.log('No input actions defined.');
    });
